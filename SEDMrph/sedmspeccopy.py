...
def find_line_match(lines, matchDict):
    '''
    Returns the line number of first line matching the position-word pairs provided within matchDict, if present within lines.
    ...
    for index in range(0, len(lines)):
        for positionKey in matchDict:
            if(len(lines[index].split()) == 0 or len(lines[index].split()) <= positionKey or lines[index].split()[positionKey] != matchDict[positionKey]):
                lineNumber = -1
                break
            else:
                lineNumber = index
...
def parse_and_fill(spec, snidoutput):
    ...
    pars = {"zmed":-1, "zmederr":-1, "agem": -1, "agemerr": -1, "Ia":0, "Ib":0, "Ic":0, "II":0, "NotSN":0, "rlap" : 0, "bestMatchType": "", "bestMatchSubtype" : "", "bestMatchRedshift" : 0}
    ...
    with open(spec, "r") as specIn:
        ...
        specLines.insert(commentLinesCount, "# SNIDFRAC_NOTSN: " + str(pars["NotSN"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDFRAC_II: " + str(pars["II"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDFRAC_IC: " + str(pars["Ic"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDFRAC_IB: " + str(pars["Ib"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDFRAC_IA: " + str(pars["Ia"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDAGEMERR: " + str(pars["agemerr"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDAGEM: " + str(pars["agem"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDZMEDERR: " + str(pars["zmederr"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDZMED: " + str(pars["zmed"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDMATCHREDSHIFT: " + str(pars["bestMatchRedshift"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDMATCHRLAP: " + str(pars["rlap"]) + "\n")
        specLines.insert(commentLinesCount, "# SNIDMATCHSUBTYPE: " + pars["bestMatchSubtype"] + "\n")
        specLines.insert(commentLinesCount, "# SNIDMATCHTYPE: " + pars["bestMatchType"] + "\n")
