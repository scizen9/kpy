...
def baseline_tests():
    ...
    print db.add_object({'name': 'test_obj', 'ra': 40., 'dec': 30., 'typedesig': 'f'})
    print db.get_from_object(['ra', 'dec'], {'name': 'test_obj'})[0] == (40, 30)
    print db.get_objects_near(39, 30, 7200)
    print db.get_object_id_from_name('defa')[0] == 2
    print db.add_elliptical_heliocentric({'object_id': 1, 'inclination': 0.1, 'perihelion_o': 0.5, 'a': 3.2, 'n': 20,
                                          'e': 0.0, 'M': 1., 'mjdepoch': 2000, 'D': 2009, 'M1': 1., 'M2': 2.,
                                          'longascnode_0': 3.})
    print db.get_from_elliptical_heliocentric(['e', 'mjdepoch'], {'D': 2009})[0] == (0, 2000)
    print db.add_hyperbolic_heliocentric({'object_id': 1, 'T': '2000-01-01', 'inclination': 0.1, 'longascnod_0': 0.5,
                                         'perihelion_o': 2., 'e': 3.2, 'q': 20., 'D': 2009, 'M1': 1., 'M2': 2.})
    print db.get_from_hyperbolic_heliocentric(['e', 'q'], {'D': 2009})[0] == (3.2, 20)
    print db.add_parabolic_heliocentric({'object_id': 1, 'T': '2000-01-01', 'inclination': 0.1, 'longascnod_0': 0.5,
                                         'perihelion_o': 2., 'q': 20., 'D': 2009, 'M1': 1., 'M2': 2.})
    print db.get_from_parabolic_heliocentric(['object_id', 'inclination'], {'M1': 1})[0] == (1, 0.1)
    ...
    print db.get_from_earth_satellite(['object_id', 'M'], {'n': 20.})[0] == (1, 3.2)
    ...
    print db.add_atomicrequest({'request_id': 1, 'exptime': 300., 'filter': 'g', 'priority': 5., 'inidate':'2017-01-01',
                                'enddate': '2017-04-03'})
    ids = db.get_from_atomicrequest(['id'], {'enddate': '2016-04-03'})
    print ids
    print db.update_atomicrequest({'id': ids[0][0], 'status': 'CANCELED'})
    print db.get_from_atomicrequest(['status'], {'id': ids[0][0]})

    print db.add_observation({'object_id': 1, 'request_id': 1, 'atomicrequest_id': 1, 'mjd': 123452.34, 'airmass': 2.2,
