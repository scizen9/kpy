...
def pull_request_from_remote(site='rsw@nera.palomar.caltech.edu',
    ...
    :return: 
    """
    remote_cmd = "scp -q %s:%s%s* %s" % (site, remote_dir, remote_files, local_dir)
...
def add_request_to_db(request, custom_dict=None, fill_by_custom_dict=False):
    ...
    # 0. If this is a delete request, cancel it from the database if it has an entry
    ...
    if fill_by_custom_dict:
        ...
        allocation_id = sedmdb.get_from_allocation(values=['id'],
