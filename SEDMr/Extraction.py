"""

SED Machine Extraction class
(c) 2014 Nick Konidaris


This class is an abstract wrapper around the extraction

"""
from numpy.polynomial.chebyshev import chebval
import numpy as np


class Extraction:
    """SED machine spectral extraction class.

    Generally there are several thousand such Extractions that are held for
    a single 2k x 2k image.

    Args:
        seg_id (int): Number from 1 to the maximum segment number. The segment
            number is generated by sextractor.
        ok (bool): Flag indicating the success of wavelength solutions.
        bkg_ok (bool): Flag indicating if the trace can be used for background.
        xrange([int,int]): From sextractor, the beginning and end of segment.
            Usually ~ 250 pixels long.
        yrange([int,int]): From sextractor, the top and bottom of the segment.
            Usually less than ~4 pixels.
        poly ([float]): Coefficients to the polynomial function tracing the
            ridge of each spectrum. Is converted to a polynomial function via
            np.poly1d. The function parameter is a pixel integer starting
            from xrange[0]
        trace_sigma (float): The width of the trace, would be used for "optimal"
           type extractions. The value of trace_sigma represents one standard
           deviation. Unit is pixel.
        spec ([float]): The extracted 1d spectrum.
        specw ([float]): The extracted 1d spectrum using the profile trace.
        specf ([float]): The extracted 1d spectrum using fractional pixels.
            (SHOULD BE DEFAULT).
        specwf ([float]): The extracted 1d spectrum using fractional pixels
            with weighting
        hg_lines ({wavelength: pixel}): An association of Hg lamp wavelength
            and pixel position. These values come from sextractor
        X_as(float): The relative position of the spaxel in arcsec
        Y_as(float): The relative position of the spaxel in arcsec
        Q_ix(int): Q coordinate of the pixel in axial units
        R_ix(int): R coordinate of the pixel in axial units
        X_pix(float): The absolute CCD position of the spaxel in pixel.
            X location is the estimated position of Halpha on the detector.
        Y_pix(float): The absolute CCD position of the spaxel in pixel.
            Y location is based on the segmentation map.
        Xhex_as(float): The hex projected X coord in arcsec
        Yhex_as(float): The hex projected Y coord in arcsec
        Xccd_as(float): The CCD X coord in arcsec
        Yccd_as(float): The CCD Y coord in arcsec

    Examples:
        You should use the values as follows:

        extractions = np.load(...)

        extraction = extractions[500] # 500 is arbitrary for this example
        pixel = 30
        X = extraction.xrange[0] + pixel
        wavelength = chebval(pixel, extraction.lamcoeff)

        print "The spectrum starting at %i and pixel %i is %f" % \
            (extraction.xrange[0], pixel, wavelength)

    """

    seg_id = None 
    ok = None
    bkg_ok = None
    is_sky = None   # Indicates if spaxel is sky
    is_obj = None   # Indicates if spaxel is object
    is_host = None  # indicates if spaxel is host
    xrange = None
    yrange = None
    xrefpix = None
    xreflam = None
    poly = None
    spec = None
    specw = None
    specf = None
    specwf = None
    hg_lines = None
    hgcoef = None     # Chebyshev coeffs of the best-fit Hg spectrum
    mdn_coeff = None  # Chebyshev coeffs of the best-fit median Hg/Xe spectra
    trace_sigma = None

    lamcoeff = None  # Chebyshev coeffs of the best-fit Hg/Xe spectra (use this)
    lamrms = None    # The RMS residual (nm) for the best wave soln.
    lamnrms = None   # The RMS normalized residual (nm) for the best wave soln.
    exptime = None

    Q_ix = None
    R_ix = None
    Xhex_as = None
    Yhex_as = None
    Xccd_as = None
    Yccd_as = None
    X_as = None
    Y_as = None
    X_pix = None
    Y_pix = None

    def get_counts(self, the_spec='specf'):
        """ Returns [wavelength, counts] spectrum.

        wavelength in nm
        counts in raw counts

        Both are around 250 pixels long

        Args:
            the_spec: Either 'spec' for the top-hat extracted
                spectrum or 'specw' for the weighted
                (so called optimal) extraction, 'specf' for the
                fractional extraction, or 'specwf' for weighted
                optimal extraciton.
        """
        lam = self.get_lambda_nm()
        if getattr(self, the_spec) is not None:
            sp = getattr(self, the_spec)
        else:
            sp = self.spec
        return [lam, sp]
    
    def get_flambda(self, the_spec='specf'):
        """ Returns [wavelength, Flambda] spectrum.

        wavelength in nm
        Flambda in Electron/nm/10 min

        Both are around 250 pixels long

        Args:
            the_spec: Either 'spec' for the top-hat extracted
                spectrum or 'specw' for the weighted
                (so called optimal) extraction, 'specf' for the
                fractional extraction, or 'specwf' for weighted
                optimal extraciton.
        """

        dlam = self.get_dlambda()
        minute = 60.0
        
        lam = self.get_lambda_nm()
        if getattr(self, the_spec) is not None:
            ss = getattr(self, the_spec)
        else:
            ss = self.spec

        el_p10m = ss*(minute*10)/self.get_exptime()
        return [lam, el_p10m/dlam]

    def get_exptime(self):
        """ Returns exptime. If None, returns 1 """

        if self.exptime is None:
            return 1
        return self.exptime

    def get_lambda_nm(self):
        """Returns lambda spectrum in nm"""
        xs = np.arange(*self.xrange)
        
        if self.lamcoeff is not None:
            lam = chebval(xs, self.lamcoeff)
        else:
            lam = chebval(xs, self.mdn_coeff)

        return lam

    def get_dlambda(self):
        """Returns dlambda spectrum """

        # Note the +1 below to handle the eating of an element
        # by diff
        xs = np.arange(self.xrange[0], self.xrange[1]+1)

        lam = chebval(xs, self.lamcoeff)
        dlam = np.abs(np.diff(lam))

        return dlam

    def __init__(self, seg_id=None, ok=None, bkg_ok=None, xrange=None,
                 yrange=None, poly=None, spec=None, specw=None, specf=None,
                 specwf=None, exptime=exptime, trace_sigma=None, hg_lines=None,
                 X_as=None, Y_as=None, Q_ix=None, R_ix=None, X_pix=None,
                 Y_pix=None,
                 Xhex_as=None, Yhex_as=None, Xccd_as=None,Yccd_as=None):

        self.seg_id = seg_id
        self.ok = ok
        self.bkg_ok = bkg_ok
        self.is_sky = False
        self.is_obj = False
        self.is_host = False
        self.xrange = xrange
        self.yrange = yrange
        self.xrefpix = 96
        self.poly = poly
        self.spec = spec
        self.specw = specw
        self.specf = specf
        self.specwf = specwf
        self.hg_lines = hg_lines
        self.exptime = exptime
        self.trace_sigma = trace_sigma
        self.X_as = X_as
        self.Y_as = Y_as
        self.Q_ix = Q_ix
        self.X_pix = X_pix
        self.Y_pix = Y_pix
        self.R_ix = R_ix
        self.Xhex_as = Xhex_as
        self.Yhex_as = Yhex_as
        self.Xccd_as = Xccd_as
        self.Yccd_as = Yccd_as

